...
class PyLayerIRCIoT(object):
 ...
 def is_ii_(self,in_str):
  def is_ii_obj_(self,in_obj):
   if not 'oid' in in_obj.keys():
    return F
   if not 'ot' in in_obj.keys():
    in_obj['ot']=N
   my_ot=in_obj['ot']
   if not 'd' in in_obj.keys():
    return F
   my_datums=in_obj['d']
   my_src=N
   if 'src' in in_obj.keys():
    my_src=in_obj['src']
   my_dst=N
   if 'dst' in in_obj.keys():
    my_dst=in_obj['dst']
  ...
  def is_ii_cont_(self,in_cont):
   if not 'mid' in in_cont.keys():
    return F
   if not 'o' in in_cont.keys():
    return F
   my_objs=in_cont['o']
 ...
 def ii_encap_int_(self,in_datumset):
  ...
  if type(i_dats)==list:
   ...
   if i_ot_cnt<i_dats_cnt:
    i_ot=N
   if i_src_cnt<i_dats_cnt:
    i_src=N
   if i_dst_cnt<i_dats_cnt:
    i_dst=N
   for i_dat in I_dats:
    if i_ii!='':
     i_ii+=','
